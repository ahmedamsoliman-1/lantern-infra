...
`192.168.215.63`. Declarative local records resolve Lantern Core and
...
- UFW rules for TCP and UDP port 53 from `192.168.215.0/24` only.
...
```sh
nslookup windows.home.arpa 192.168.215.253
nslookup dashboard.home.arpa 192.168.215.253
nslookup example.com 192.168.215.253
```
...
`192.168.215.63` (the hotspot gateway). No router-wide DNS change is part of
this phase.
